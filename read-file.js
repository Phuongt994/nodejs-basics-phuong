...
const main = () => {
  ...
  try {
    // Task 1
    const productJSON = fs.readFileSync('./commons/products.json', 'utf8');
...
const printTemplate = products => {
  products.forEach(val => {
    ...
    const fromNow = formatDistance(val.dateUpdated, new Date(), { locale: vi });
...
const convertWorksheet = products => {
  ...
  // **Taken from sheet: config columns width
  prodWs['!cols'] = [
    { width: 20 },
    { width: 15 },
    { width: 20 },
